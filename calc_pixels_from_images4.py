...
capturedir = workdir+"newcaps/"+cam+"/"
...
datafiles = [p for p in os.listdir(elementcapdir) if p.startswith('data_') and p.endswith('.csv')]
print('Have these datafiles in {:s}'.format(elementcapdir));
print(datafiles);

datafiles_captures = [os.path.splitext(p)[0].split('_')[1] for p in datafiles]

remaining_captures = sorted(list(set(captures)-set(datafiles_captures)))
print('Remaining captures that need pixel detection:')
print(remaining_captures)

#%% Test 1 2 3 4 5 6 7 8
mycapname = remaining_captures[0];
#mycapname = captures[1];
...
info_index = [int(s.split('_')[2]) for s in bases]
...
abort = False;
#for cnt,(fon,foff) in enumerate(zip(images_on,images_off)):
#cnt = 0;
while True:
    fon = images_on[cnt];
    foff = images_off[cnt];
    ...
    unum = info_uni[cnt];
    index = info_index[cnt];
    model = info_model[cnt];
    ...
    rgbdiff = cv2.subtract(masked_image,masked_image_off);
    graydiff1 = cv2.cvtColor(rgbdiff, cv2.COLOR_BGR2GRAY)
    
    ##### BLUR GRAYDIFF
    blurredgraydiff = cv2.GaussianBlur(graydiff1, (15, 15), 0)
    
    #cv2.imshow("diff",diff);
    #cv2.imshow('graydiff',graydiff1)
    #cv2.imshow('blurredgraydiff',blurredgraydiff)
    #break;
    
    ####PICK OUT RED/WHITE COLORS IN DIFFERENCE IMAGE
    def maskRed(img_rgb):
        # convert to HSV
        ...
        output_img[np.where(masktotal==0)] = 0
    ...
    # cv2.imshow('whtmsk',whtmsk);
    # cv2.imshow('redmsk',redmsk);
    
    
    #### PROCESS WITH ERODES/DILATES
    def proc1(thresh):
    ...
    # right
    whtcnts = cv2.findContours(whtmskfin.copy(), cv2.RETR_LIST,
        cv2.CHAIN_APPROX_SIMPLE)[0];
    redcnts = cv2.findContours(redmskfin.copy(), cv2.RETR_LIST,
        cv2.CHAIN_APPROX_SIMPLE)[0];
    if(len(whtcnts)>0):
    ...
    def contourIntersect(original_image, contour1, contour2):
        ...
        print('\t\t          rc Nonzero:',np.count_nonzero(image1));
        print('\t\t          wc Nonzero:',np.count_nonzero(image2));
        print('\t\tIntersection Nonzero:',np.count_nonzero(intersection));
        #print('Intersection Zero   : ':np.cou);
        return intersection.any(),np.count_nonzero(image1),np.count_nonzero(image2),np.count_nonzero(intersection)
    ...
    # #cv2.imshow('redresult',redresult);
    ...
    # #thresh = cv2.threshold(graydiff.copy(), 50, 255,cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]
    # thresh = cv2.threshold(graydiff.copy(), 150, 255,cv2.THRESH_BINARY_INV)[1]
    ...
    cv2.putText(imageAnno, 
        text = 'U {:d} pxnum {:03d} {:s} success={:d}'.format(unum,index,model,success),
        org = (100,text_overlay_y),
    ...
    if(len(cnts) == 0 or len(cnts)>1):
        ...
        need_mouse_result = True;
        results[(unum,index)] = [unum,index,0,0,False]; # prepopulate with zero
        
        cv2.putText(imageAnno, 
            text = 'NEED RESULT'.format(),
            org = (1300,text_overlay_y),
            fontFace = cv2.FONT_HERSHEY_DUPLEX,
            fontScale = 2,
            color = (255,255,0),
            thickness = 2,
    ...
    while not done:
        ...
        if(key==27): # ESC
            ...
            if(prevcnt!=0):
                ...
                cv2.imshow(windowName, imageAnno);
            pass;
        elif(key==32 or key==109): # space or "m"
        ...
        #     break;
    # else:
    #     # automatically found, show position
    #     done = False;
    #     while not done:
    #         key = cv2.waitKey(50)
    #         print(key)
    ...
    #             abort = True;
    #             break;
    #         elif(key==31):  # left bracket
                
    if(undolast):
        cnt -= 1;
    else:
        cnt += 1;
    if(abort or (cnt>=len(images_on))):
        break;
