...
class CaptureSet:
    ...
    def morePoseCalculations(self):
        ...
        # projection matrix
        Pmat = np.matrix(np.hstack((R,ds.tvec.T)));
...
for ds in datasets:
    ...
    #cone = cone.rotateY(-thetay,rad=True,locally=True);
    #cone = cone.rotateZ(0,rad=True,locally=True);
    #cone = cone.rotateX(thetax,rad=True,locally=True);
    #cone = cone.rotateY(-thetay,rad=True,locally=True);
    #cone = cone.rotateZ(0,rad=True,locally=True);
    #cone = cone.rotate(theta,axis=v,point=P,rad=True);
    cones.append(cone);
    ...
    #Lcam=mtx.dot(np.hstack((R,ds.tvec.T)));
    Lcam=mtx.dot(Pmat);
    px=ds.data.loc[(universe,index)]['x']
    py=ds.data.loc[(universe,index)]['y']
    # px=1000
    # py=1019
    Z=0
    X=np.linalg.inv(np.hstack((Lcam[:,0:2],np.array([[-1*px],[-1*py],[-1]])))).dot((-Z*Lcam[:,2]-Lcam[:,3]))
